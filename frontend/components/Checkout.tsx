      ...
      <div>
        ...
        {orderStatus === null && (
          <form onSubmit={handleSubmit}>
            ...
            <button type="submit" className="bg-black mt-3 text-white rounded-xl px-4 py-2">
              Place Order
            </button>
          </form>
        )}
        {orderStatus === 'pending' && (
          <div className="flex flex-col items-center">
            ...
            <p className="text-lg">Please wait while we place your order...</p>
          </div>
        )}
        {orderStatus === 'success' && (
          <div className="flex items-center">
            <div>
              ...
              <p className="mb-4">
                You will receive a text confirmation shortly. You can pick up your product at{' '}
                <strong>{productInfo.dispensary}</strong> located at 123 Main St, Anytown, USA. Please bring a valid ID.
              </p>
            </div>
          </div>
        )}
    ...
    />
        ...
        <div className="mt-3">
          <div className="flex justify-between">
            ...
            <p className="text-xs text-center">{productInfo.product_name.length > 18 ? `${productInfo.product_name.substring(0,18)}... `: productInfo.product_name}</p>
            <p className="text-sm text-center">${totalPrice}</p>
          </div>
          {discountAmount !== '0.00' && (
            <div className="flex flex-row space-x-7 items-center mt-2 justify-end">
              <p className="text-sm text-center font-bold text-gray-700">Promo Code ({promoCode})</p>
          ...
          <div className="border-t mt-5 pt-2 flex flex-row justify-between items-center">
            ...
            <p className="font-bold text-gray-600">${discountedPrice}</p>
          </div>
        </div>
      </div>
    </div>
...
export default function CheckoutCard({ productInfo }: { productInfo: ProductInfo }) {
  ...
  const totalPrice = (parseFloat(productInfo.price.replace(/[^0-9.-]+/g, '')) * quantity).toFixed(2);
  ...
  return (
    <>
      {!showCheckoutForm && (
        ...
        <div className="flex flex row items-center  border mx-auto rounded-xl"> 
          <div className="flex flex-col p-5 space-y-3 w-full">
            <p className="text-gray-500">{productInfo.brand_name}</p>
            <p className="font-bold text-xl">{productInfo.product_name}</p>
            <p className="text-l text-green-800 font-bold">{productInfo.price}</p>
            <div className="flex flex-row">
              <label htmlFor="quantity" className="text-gray-500"></label>
              <select
                id="quantity"
              ...
              >
                <h1 className="text-white text-sm">Proceed to Checkout ${totalPrice}</h1>
              </button>
            </div>
            </div>
            <img
      ...
      {showCheckoutForm && (
        <CheckoutForm productInfo={productInfo} quantity={quantity} totalPrice={totalPrice} />
      )}
    </>
  );
}
