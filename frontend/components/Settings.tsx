...
export default function Settings() {
    ...
    const [selectedProducts, setSelectedProducts] = useState<{ price: string; cbdContent: string; strain: string; productType: string; }[]>([]);
    ...
    const handleAddProduct = () => {
        ...
        const allFieldsFilled = Object.values(newProduct).every(field => field !== '');
            ...
            <div className="bg-white rounded-lg p-6">
                ...
                <div className="mb-6">
                    <label className="inline-flex relative items-center cursor-pointer">
                        ...
                        <span className="ml-3 text-md font-medium text-gray-900 dark:text-gray-300">Plug Mode</span>
                        <PowerIcon />
                    </label>
                    <p className="opacity-60">     Never miss out on your favorite products again! With Plug Mode, simply toggle it on and enter the specific product you&apos;re looking for. Our AI Budtender will keep an eye out and notify you as soon as the product becomes available. Stay in the loop and get exactly what you need, when you need it.</p>
                </div>
                {plugMode && (
    <div className="p-6 bg-white border rounded-xl mb-6">
        <h2 className="text-2xl font-bold mb-6 text-gray-900">Products</h2>
        {selectedProducts.map((product, index) => (
            <div key={index} className="mb-4 p-4 flex flex-row justify-between border-b border-gray-200">
                ...
                <p className="text-gray-700">
                    ...
                    <p className="text-gray-700">
                    <div className="flex flex-col">
                    <span className="font-medium "> CBD:</span> {product.cbdContent}
                    </div>
                    </p>
                    <p className="text-gray-700">
                    <div className="flex flex-col">
        ...
        <div className="mb-2">
            <label className="block text-gray-700 text-sm font-bold mb-2" htmlFor="cbdContent">
                CBD Content (%)
            </label>
            <input
                className=" appearance-none border rounded-xl w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:ring-2 focus:ring-gray-200"
                ...
                onChange={(e) => setNewProduct({ ...newProduct, cbdContent: e.target.value })}
                disabled={anyCBD}
            />
            <label className="inline-flex relative items-center mt-2 cursor-pointer">
                        <input type="checkbox" className="sr-only peer" checked={anyCBD} onChange={handleCBDToggle} />
                        <div className="w-11 h-6 bg-gray-200 rounded-full peer  dark:peer-focus:ring-gray-800 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all dark:border-gray-600 peer-checked:bg-gray-600"></div>
                        <span className="ml-3 text-md font-medium text-gray-900 dark:text-gray-300">Any CBD Content</span>
                    </label>
        </div>
     
        <div className="mb-4">
            <label className="block text-gray-700 text-sm font-bold mb-2" htmlFor="strain">
                Strain
            </label>
            <select
                className=" border rounded-xl py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:ring-2 focus:ring-gray-200"
                ...
                onChange={(e) => setNewProduct({ ...newProduct, strain: e.target.value })}
            >
                <option>Any</option>
...
<div className="flex items-center justify-between mt-6">
    <button className="bg-gray-800 hover:bg-gray-900 text-white font-bold py-2 px-4 rounded-xl focus:outline-none focus:ring-2 focus:ring-gray-300" type="button">
        Save
    </button>
    <button className="bg-transparent hover:bg-red-500 rounded-xl text-red-700 font-semibold hover:text-white py-2 px-4 border border-red-500 hover:border-transparent rounded-md focus:outline-none focus:ring-2 focus:ring-red-300" type="button">
        Cancel
    </button>
</div>
