    ...
    interface ApiData {
      ...
      const filteredData = data.filter((product) => {
        ...
        const dispensaryMatch =
          selectedDispensaries.length === 0 || selectedDispensaries.includes(product.dispensary);
      ...
      return (
        <div className="flex flex-row">
          <div className="w-40 border-r h-[calc(100vh-64px)] space-y-10 pt-10 px-3 flex flex-col">
  ...
  {types
    ...
    .map((type) => (
      <div key={type} className="flex items-center">
        <input
          type="checkbox"
          className="form-checkbox h-3 w-3 text-gray-200"
...
<div className="flex flex-col border-b pb-5 space-y-2">
  ...
  {dispensaries
    ...
    .map((dispensary) => (
      <div key={dispensary} className="flex items-center">
        <input
          type="checkbox"
          className="form-checkbox h-3 w-3 text-gray-600"
          checked={selectedDispensaries.includes(dispensary)}
          onChange={() => handleDispensaryChange(dispensary)}
        />
        <span className="ml-2 text-xs">{dispensary}</span>
      </div>
    ))}
</div>
                ...
                {filteredData.map((productInfo) => (
                  ...
                  <li key={productInfo.id} className="border-b p-4">
                    ...
                    />                      
                      ...
                      <div className="bg-custom-gray shadow-sm w-40 h-20 rounded-xl text-center flex items-center justify-center">                          {productInfo.sales && productInfo.sales !== "null" ? (
                              <>
